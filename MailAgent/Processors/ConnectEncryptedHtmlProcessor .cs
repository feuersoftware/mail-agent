...
{
    public class ConnectEncryptedHtmlProcessor : IMailProcessor
    {
        ...
        private readonly ConnectPlainHtmlProcessor _fallbackProcessor;

        public ConnectEncryptedHtmlProcessor(
        ...
        public async Task ProcessMailAsync((MimeMessage message, SiteModel site) siteMail)
        {
            try
            {
                ...
                var textPart = (MimePart)entity.BodyParts.Single(p => p.ContentType.MimeType == "text/html");
                ...
                if (textPart.ContentTransferEncoding == ContentEncoding.Base64)
                {
                    byte[] data = Convert.FromBase64String(contentQuotedPrintableEncoded);
                ...
                else
                {
                    ...
                    decodedString = QuotedPrintableConverter.Decode(contentQuotedPrintableEncoded, decryptedPartEncoding);
                }
